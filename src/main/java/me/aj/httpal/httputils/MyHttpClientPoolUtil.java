package me.aj.httpal.httputils;
...
public class MyHttpClientPoolUtil {
	...
	CloseableHttpClient singletonHttpClient=null;
	...
	private CloseableHttpClient getHttpClientInstance() {
		if(this.singletonHttpClient == null){
			singletonHttpClient = new DefaultHttpClient();
		}
		return this.singletonHttpClient;
	}
	...
	private HttpRequestBase setHeaders(MyHTTPServletRequest httpRequest) {
		...
		if ("POST".equals(httpRequest.getMethod())) {
			...
			httpMethod = new HttpPost(getTargetURL(httpRequest.getService()));
		} else if ("GET".equals(httpRequest.getMethod())) {
			httpMethod = new HttpGet(getTargetURL(httpRequest.getService()));
		}else if("HEAD".equalsIgnoreCase(httpRequest.getMethod())){
			httpMethod = new HttpHead(getTargetURL(httpRequest.getService()));
		}
	...
	public boolean isItUpRightNow(MyHTTPServletRequest httpRequest) {
		URL url=null;;
		try {
			url = new URL(httpRequest.getService());
